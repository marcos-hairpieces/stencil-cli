...
class StencilStart {
    constructor({
        ...
        // storeSettingsApiClient and stencilPushUtils are consumed via the bootstrap below
        // rather than directly, so they are not kept as fields.
        // Built from the already-injected collaborators so test doubles keep flowing through.
        this._bootstrap =
            bootstrap ||
            new StencilBootstrap({
                themeApiClient,
    ...
    async getStoreSettingsLocale(cliOptions, stencilConfig) {
        return this._bootstrap.fetchStoreLocale(stencilConfig, cliOptions, this.storeHash);
    }

    async getChannelUrl(stencilConfig, cliOptions) {
        this.storeHash = await this._bootstrap.resolveStoreHash(stencilConfig);
        return this._bootstrap.resolveChannelUrl(stencilConfig, cliOptions, {
            // `start` runs in a terminal, so an ambiguous channel can be resolved by asking.
            interactive: true,
